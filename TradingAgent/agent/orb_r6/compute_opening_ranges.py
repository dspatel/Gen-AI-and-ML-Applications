...
def ensure_opening_ranges_table(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS ux_opening_ranges_symbol_date_minutes_interval
        ON opening_ranges(symbol, session_date_cst, or_minutes, interval);
        """
    )
...
def compute_opening_ranges(
    ...
    open_dt = pd.to_datetime(df["open_ts_cst"], utc=True).dt.tz_convert(CST)
