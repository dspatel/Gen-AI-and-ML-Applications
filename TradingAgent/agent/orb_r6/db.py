...
def connect(db_path: str) -> sqlite3.Connection:
    ...
    if not p.is_absolute():
        # db.py -> orb_r6 -> agent -> project root
...
def init_db(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS ux_candles_symbol_interval_open_ts_cst
        ON candles(symbol, interval, open_ts_cst);
        """
    )
    conn.commit()
...
def upsert_candles(conn: sqlite3.Connection, rows: Iterable[CandleRow]) -> Tuple[int, int]:
    ...
    ON CONFLICT(symbol, interval, open_ts_utc) DO UPDATE SET
        close_ts_utc=excluded.close_ts_utc,
        open_ts_cst=excluded.open_ts_cst,
        close_ts_cst=excluded.close_ts_cst,
        cst_date=excluded.cst_date,
        open=excluded.open,
        high=excluded.high,
        low=excluded.low,
        ...
        ingested_at_cst=excluded.ingested_at_cst;
    """
