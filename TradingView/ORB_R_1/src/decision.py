...
def compute_trade_decision(metrics: Dict[str, Any], direction: str, rules_cfg: Dict[str, Any]) -> DecisionResult:
    """Backward-compatible wrapper used by tools.

    Parameters
    ----------
    metrics: dict
        Feature dictionary (RR + breakout quality).
    direction: str
        'UP' or 'DOWN'
    rules_cfg: dict
        Parsed decision rules YAML.
    """
    return decide(metrics=metrics, direction=direction, rules_cfg=rules_cfg)
