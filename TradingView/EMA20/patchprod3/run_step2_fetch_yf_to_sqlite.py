...
def main():
    ...
    # In TEST mode we often replay an historical "as-of" day after hours.
    # In that case, use today's symbols file name.
    run_date = today_ymd()
    symbols_path = os.path.join(CFG.SYMBOLS_DIR, f"symbols_{run_date}.csv")

    # If the exact file isn't present, fall back to the latest symbols_*.csv in the symbols dir.
    # This keeps Step2 usable even when you didn't run Step1 in the current session.
    if not os.path.exists(symbols_path):
        try:
            from utils.io_utils import find_latest_file
            fallback = find_latest_file(CFG.SYMBOLS_DIR, prefix="symbols_", suffix=".csv")
        except Exception:
            fallback = ""
        if fallback and os.path.exists(fallback):
            print(f"[WARN] Symbols file for {run_date} not found. Falling back to latest: {os.path.basename(fallback)}")
            symbols_path = fallback
        else:
