...
When adding new fields used by outputs (e.g., `scan_alerts`), prefer **DB-first**:
- write to `alerts_log` at the moment the event happens (LIVE)
- fill any end-of-day-only fields during finalization (EOD)
- export from the ledger (never re-derive from scratch)
...
## 8) Secrets hygiene
- Never hardcode Discord webhooks (or any secrets) in the repo.
- Use environment variables:
  - `DISCORD_WEBHOOK_URL` (PROD)


- Keep LIVE alerts (`alerts_log`) and EOD scan results separate. EOD scan results must go to `EOD_DB_PATH` and never be inserted into `alerts_log`.
