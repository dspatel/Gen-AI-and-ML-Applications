# EMA20 Anchored Breakout Scanner (PRODUCTION)

> Production has **no test/replay mode**. For historical replays or sandbox runs, use the separate **Scanner_TEST** project.
...
- If `REARM_ON_REENTRY = True`, we re-arm when price re-enters the **primary window (configurable, e.g., 35D)**:
...
flowchart TD
  ...
  C --> D[If new cross: freeze 7D and secondary window (configurable)s]
...
## Verify runtime environment (recommended)

Before running `daily_runner.py` (or any step script), you can print the **effective** configuration
that will be used at runtime (resolved DB paths, output folders, and Discord routing):

```bash
python tools/print_runtime_config.py
```

Use this to confirm you are not writing to PROD by accident when you intended to run in TEST.
...
### scan_alerts CSV includes trigger OHLC for simulations
`run_step3_scan_from_sqlite.py` exports `data/outputs/scan_alerts_YYYY-MM-DD.csv` from the SQLite ledger (`alerts_log`).
This means the file contains:
- **Trigger candle OHLC** (intraday candle that fired the alert)
- **Day OHLC at alert time** (as known from intraday bars)
- **Final daily OHLC** (filled during EOD finalization from daily candles)
...
- `daily_runner.py` — run morning prep / live / EOD workflows
- `tools/db_inspect.py` — quick SQLite sanity checks
- `tools/sqlite_checks.sql` — SQLite CLI checks
- `tools/multi_window_backtest.py` — compare multiple window lengths over a historical range

### Daily runner

Run the full day lifecycle with one command:
...
This will:
- Run morning prep (Step1 → Step2 → Step3 as-of last trading day; Discord disabled for prep)
- Start live monitoring at market open (America/Chicago)
- Run end-of-day finalization after close (Step2 → Step3; Discord controlled by config)


### scan_alerts CSV column order
The `scan_alerts_YYYY-MM-DD.csv` export is sourced from the alerts ledger and is written with a readability-first column order (Symbol/Signal/Times, Candle OHLC, Day OHLC, EMA, windows, metrics). Any additional fields are appended to the right; no fields are dropped.


## Outputs (daily)

- `data/outputs/scan_all_<YYYY-MM-DD>.csv` – full scan rows
- `data/outputs/eod_scan_alerts_<YYYY-MM-DD>.csv` – EOD daily-close breakout signals (from Step3 scan)
- `data/outputs/live_alerts_<YYYY-MM-DD>.csv` – intraday LIVE trigger ledger export (from alerts_log)

## Databases

- **Production DB**: `data/db/scanner.db` (daily bars cache, symbol_state, LIVE alerts ledger `alerts_log`)
- **EOD Scan DB**: `data/cache/eod_scan.sqlite` (EOD scan alerts only; separate from LIVE ledger)
