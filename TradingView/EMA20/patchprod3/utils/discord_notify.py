...
def send_discord_file(webhook_url: str, file_path: str, content: str = "", timeout: int = 30) -> None:
    """Send a Discord webhook message with a file attachment.

    Notes:
    - Discord webhooks support multipart/form-data with a `file` field.
    - Keep attachments small; CSV alert files should be tiny.
    """
    if not webhook_url:
        return
...
def _fpct(x: Any, ndp: int = 2) -> str:
    """Format a fraction (0.1234) as a percentage string (12.34%)."""
    try:
        return f"{float(x) * 100:.{ndp}f}%"
    except Exception:
        return str(x)
...
def format_alert_message(alert: Dict[str, Any], env: str = "LIVE") -> str:
    ...
    lines: List[str] = []
    # Small UX improvements (keep full detail):
    # - Use LONG/SHORT colored emoji
    # - Keep numeric values rounded
    # - Show break % as both percent and raw fraction to avoid ambiguity
    sig_u = str(signal).upper() if signal is not None else ""
    emoji = "🟢" if sig_u == "LONG" else "🔴" if sig_u == "SHORT" else "🚨"
    lines.append(f"{emoji} EMA20 Anchored Breakout ({env})")
    lines.append("")
    ...
    # Window lengths
    d1 = (
        alert.get("PrimaryWindowDaysUsed")  # NEW (live tracker)
        or alert.get("WindowDaysPrimary")
        or alert.get("window_days_primary")
        or alert.get("WindowDays_Primary")
    )
    d2 = (
        alert.get("SecondaryWindowDaysUsed")  # NEW (live tracker)
        or alert.get("WindowDaysSecondary")
        or alert.get("window_days_secondary")
        or alert.get("WindowDays_Secondary")
    )
    ...
    if d1:
        ...
        if bp1 is not None:
            lines.append(f"Break % of {int(d1)}D Range: {_fpct(bp1)}")
        lines.append("")
    ...
    if d2:
        ...
        if bp2 is not None:
            lines.append(f"Break % of {int(d2)}D Range: {_fpct(bp2)}")
        lines.append("")
    ...
    # EMA20 recent cross stats (feature engineering)
    x = alert.get("Ema20CrossLookbackTD") or alert.get("ema20_cross_lookback_td")
    ct = alert.get("Ema20CrossCountTotal") or alert.get("ema20_cross_count_total")
    cb = alert.get("Ema20CrossCountBull") or alert.get("ema20_cross_count_bull")
    cr = alert.get("Ema20CrossCountBear") or alert.get("ema20_cross_count_bear")
    ds = alert.get("Ema20CrossDaysSinceLast") or alert.get("ema20_cross_days_since_last")
    den = alert.get("Ema20CrossDensity") or alert.get("ema20_cross_density")
    if x is not None and ct is not None:
        try:
            lines.append("")
            lines.append(f"EMA20 crosses (last {int(x)} TD): total={int(ct)} | bull={int(cb or 0)} | bear={int(cr or 0)}")
            if ds is not None:
                lines.append(f"Days since last cross: {ds}")
            if den is not None:
                lines.append(f"Cross density: {_fnum(den, 3)} per TD")
        except Exception:
            pass
