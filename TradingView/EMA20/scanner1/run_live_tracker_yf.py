...
def get_session_times_chicago() -> tuple[datetime, datetime] | tuple[None, None]:
    """Return today's NYSE regular session open/close in America/Chicago.

    exchange_calendars has some API drift across versions:
    - `cal.schedule` can be a callable or a DataFrame
    - columns can be `open/close` or `market_open/market_close`

    This helper is defensive so live tracking doesn't break due to library upgrades.
    """
    tz = ZoneInfo(CFG.TIMEZONE)
    cal = xcals.get_calendar("XNYS")

    today_str = pd.Timestamp.now(tz=tz).strftime("%Y-%m-%d")
    ...
    market_open = pd.Timestamp(open_ts).to_pydatetime().astimezone(tz)
    market_close = pd.Timestamp(close_ts).to_pydatetime().astimezone(tz)
...
def _to_float(x) -> float:
    """Convert pandas/numpy scalars (or 1-element Series) to float safely."""
    import numpy as np
...
def main():
    ...
    while True:
        ...
        for sym in symbols:
            ...
            if need_refresh:
                # Backward/forward compatible window-day config
                win7_days = getattr(CFG, "WINDOW_DAYS_SHORT", getattr(CFG, "WINDOW_DAYS", 7))
                win21_days = getattr(CFG, "WINDOW_DAYS_LONG", 21)

                win7 = compute_anchored_window_before_cross(daily, latest_cross_date, win7_days)
                win21 = compute_anchored_window_before_cross(daily, latest_cross_date, win21_days)
            ...
            # ---- Normalize intraday bars so time is ALWAYS a tz-aware DatetimeIndex ----
            # Flatten MultiIndex columns if present (sometimes happens)
            if isinstance(intra.columns, pd.MultiIndex):
                intra.columns = [c[0] for c in intra.columns]

            # If Timestamp exists as a column, set it as index
            if "Timestamp" in intra.columns:
                intra["Timestamp"] = pd.to_datetime(intra["Timestamp"], errors="coerce", utc=True)
                intra = intra.dropna(subset=["Timestamp"]).set_index("Timestamp")
            elif "Datetime" in intra.columns:
                intra["Datetime"] = pd.to_datetime(intra["Datetime"], errors="coerce", utc=True)
                intra = intra.dropna(subset=["Datetime"]).set_index("Datetime")
            else:
                # Otherwise assume the index is the timestamp (yfinance default)
                if not isinstance(intra.index, pd.DatetimeIndex):
                    intra.index = pd.to_datetime(intra.index, errors="coerce", utc=True)
                # Drop bad rows if any
                intra = intra[~intra.index.isna()]

            # Ensure tz-aware UTC then convert to Chicago (CST/CDT)
            if intra.index.tz is None:
                intra.index = intra.index.tz_localize("UTC")
            intra.index = intra.index.tz_convert(tz)

            # Sort
            intra = intra.sort_index()

            # Use last completed bar by default to avoid firing on an in-progress candle
            use_completed = bool(getattr(CFG, "LIVE_USE_LAST_COMPLETED_BAR", True))
            bar_idx = -2 if use_completed and len(intra) >= 2 else -1

            candle_ts = intra.index[bar_idx]
            candle_time_str = candle_ts.strftime("%Y-%m-%d %H:%M:%S %Z")

            latest_bar = intra.iloc[bar_idx]
            live_price = _to_float(latest_bar["Close"])
            live_high = _to_float(intra["High"].max())
            live_low  = _to_float(intra["Low"].min())

            # Guardrail: if candle time is epoch-ish, something is wrong with intraday timestamps
            if pd.Timestamp(candle_ts).year < 2000:
                raise RuntimeError(f"Invalid candle timestamp detected: {candle_ts}")
            ...
            alert = {
                ...
                "CandleTimestamp": pd.Timestamp(candle_ts).isoformat(),
