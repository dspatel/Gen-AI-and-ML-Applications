...
def format_alert_message(alert: Dict[str, Any], env: str = "LIVE") -> str:
    ...
    if d1:
        ...
        if bp1 is not None:
            # keep both so we don't lose precision/meaning
            lines.append(f"Break % of {int(d1)}D Range: {_fpct(bp1)} ({_fnum(bp1, 6)})")
        lines.append("")
    ...
    if d2:
        ...
        if bp2 is not None:
            lines.append(f"Break % of {int(d2)}D Range: {_fpct(bp2)} ({_fnum(bp2, 6)})")
        lines.append("")
