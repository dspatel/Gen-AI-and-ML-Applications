...
class CFG:
    ...
    WINDOW_DAYS_SHORT: int = 7  # DEPRECATED (kept for compatibility)
    WINDOW_DAYS_LONG: int = 21  # DEPRECATED (kept for compatibility)

    # Primary/Secondary window lengths used for frozen pre-cross windows
    ...
    # =========================
    # Enable/disable Discord messaging. By default, this is ON.
    # You can hard-disable by setting:  $env:EMA_DISCORD_ENABLED="0"
    DISCORD_ENABLED: bool = os.getenv("EMA_DISCORD_ENABLED", "1").strip().lower() in ("1", "true", "yes", "y", "on")
    DISCORD_ENV: str = "PROD"  # "TEST" or "PROD" (shown in message title)

    # Webhook URL MUST be provided via environment variable.
    ...
    # Preferred: upload the generated `scan_alerts_YYYY-MM-DD.csv` to Discord as a file attachment.
    # This keeps full detail without cluttering chat.
    DISCORD_SEND_EOD_ALERTS_CSV: bool = True
    # If True, upload the CSV only when at least 1 alert exists.
    DISCORD_UPLOAD_CSV_ONLY_IF_ALERTS: bool = True
    DISCORD_MAX_ALERTS: int = 10

    # "No confusion" banners (recommended): print + optional Discord summary of key knobs at start/end
    ...
    # =========================
    TEST_MODE: bool = os.getenv("EMA_TEST_MODE", "0").strip().lower() in ("1","true","yes","y","on")
    ASOF_DATE: str = os.getenv("EMA_ASOF_DATE", "").strip()  # YYYY-MM-DD when EMA_TEST_MODE=1

    # "read_only" = never writes state; "sandbox" = writes to symbol_state_test table
    TEST_STATE_MODE: str = os.getenv("EMA_TEST_STATE_MODE", "read_only").strip()
    ...
    LIVE_CLOSE_GRACE_MINUTES: int = 2   # allow last bar to settle
    LIVE_PRINT_WAIT_STATUS_EVERY_SECONDS: int = 300  # print "waiting..." every 5 min

    LIVE_SESSION_OPEN: str = "08:30"   # RTH open in America/Chicago
    LIVE_SESSION_CLOSE: str = "15:00"  # RTH close in America/Chicago
