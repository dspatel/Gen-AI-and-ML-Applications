# ============================================================
# Module E-M: Trade Execution (Momentum ORB + EMA20)
#
# Inputs:
...
#   data/SPY_daily_summary_momentum.csv
#
# Execution assumptions:
# - Entry fills at NEXT BAR OPEN after entry_signal candle closes.
# - Stop can fill intrabar using high/low.
# - EMA exit uses 1 consecutive close beyond EMA (momentum = faster).
#   Exit fills at NEXT BAR OPEN after the confirming close.
# - EOD exit fills at the 15:55 bar CLOSE.
#
# Run:
#   python test_execution_momentum_spy.py
# ============================================================
...
class ExecConfig:
    ...
    allow_one_trade_per_day: bool = False # momentum can take >1/day
...
def main():
    ...
    in_path = os.path.join(os.getcwd(), "data", "SPY_30d_5m_yahoo_ema20_orb_signals_momentum.csv")
    trades_path = os.path.join(os.getcwd(), "data", "SPY_trades_momentum.csv")
    daily_path = os.path.join(os.getcwd(), "data", "SPY_daily_summary_momentum.csv")

    print("\n[Module E-M] Loading momentum signals CSV:")
    print(f"  {in_path}")
    ...
    print(f"\n✅ Trades generated (momentum): {len(trades)}")
    if not trades.empty:
        show_cols = [
            "session_date", "side",
    ...
    print("\nNext: run Module F-M (decision vs reality) for momentum.\n")
