# ============================================================
# Module G v2: $10,000 Equity Simulation
#
# Inputs:
...
#   data/SPY_equity_curve_momentum_v2.csv
#   data/SPY_equity_summary_comparison_v2.csv
# ============================================================
...
def main():
    ...
    mom_v2_path = os.path.join(base, "SPY_trades_momentum_v2.csv")

    cons_trades = load_trades(cons_path)
    mom_trades = load_trades(mom_v2_path)

    cons = simulate_equity(cons_trades, cfg, "conservative")
    mom = simulate_equity(mom_trades, cfg, "momentum_v2_gated")

    cons_curve_path = os.path.join(base, "SPY_equity_curve_conservative.csv")
    mom_curve_path = os.path.join(base, "SPY_equity_curve_momentum_v2.csv")
    ...
    summary_path = os.path.join(base, "SPY_equity_summary_comparison_v2.csv")
    summary.to_csv(summary_path, index=False)

    def fmt(x, nd=2):
        return "NA" if x is None or (isinstance(x, float) and pd.isna(x)) else round(float(x), nd)

    print("\n=== $10,000 Equity Simulation (Conservative vs Momentum v2) ===")
