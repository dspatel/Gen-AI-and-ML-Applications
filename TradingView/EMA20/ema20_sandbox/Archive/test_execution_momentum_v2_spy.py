# ============================================================
# Module E-M v2: Trade Execution (Momentum GATED v2)
#
# Input:
#   data/SPY_30d_5m_yahoo_ema20_orb_signals_momentum_v2.csv
#
# Outputs:
#   data/SPY_trades_momentum_v2.csv
#   data/SPY_daily_summary_momentum_v2.csv
#
# Momentum exits:
# - EMA exit uses 1 consecutive close beyond EMA (fast).
# - Entry fills at NEXT BAR OPEN after entry_signal candle.
# - Stop fills intrabar using high/low.
# - EOD exit at 15:55 close.
# ============================================================
...
class ExecConfig:
    ...
    ema_exit_confirm_closes: int = 1      # momentum
    allow_one_trade_per_day: bool = True  # keep 1/day for v2 controlled test
...
def main():
    ...
    in_path = os.path.join(os.getcwd(), "data", "SPY_30d_5m_yahoo_ema20_orb_signals_momentum_v2.csv")
    trades_path = os.path.join(os.getcwd(), "data", "SPY_trades_momentum_v2.csv")
    daily_path = os.path.join(os.getcwd(), "data", "SPY_daily_summary_momentum_v2.csv")

    print("\n[Module E-M v2] Loading momentum v2 signals CSV:")
    print(f"  {in_path}")
    ...
    print(f"\n✅ Trades generated (momentum v2 gated): {len(trades)}")
    if not trades.empty:
        show_cols = [
            "session_date", "side",
    ...
    print("\nNext: run Module F-M v2 (decision vs reality) and $10k sim.\n")
