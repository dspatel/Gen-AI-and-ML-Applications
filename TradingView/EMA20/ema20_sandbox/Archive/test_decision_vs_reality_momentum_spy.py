# ============================================================
# Module F-M: Decision vs Reality Analyzer (Momentum)
#
# Inputs:
#   data/SPY_30d_5m_yahoo_ema20_orb_signals_momentum.csv   (Module D-M)
#   data/SPY_trades_momentum.csv                          (Module E-M)
#
# Output:
#   data/SPY_decision_vs_reality_momentum.csv
#
# Same evaluation logic as conservative:
# - Evaluate each ORB-confirmed day with fixed horizon (default 60 min)
# - If traded: use actual entry/stop as reference
# - If skipped: build ORB reference plan (entry after breakout, stop at opposite OR boundary)
# ============================================================
...
def get_trade_for_day(trades: pd.DataFrame, session_date: str) -> Optional[pd.Series]:
    ...
    if d.empty:
        return None
    # momentum may have multiple trades/day; evaluate the FIRST trade for day-level signal quality
...
def main():
    ...
    signals_path = os.path.join(os.getcwd(), "data", "SPY_30d_5m_yahoo_ema20_orb_signals_momentum.csv")
    trades_path = os.path.join(os.getcwd(), "data", "SPY_trades_momentum.csv")
    out_path = os.path.join(os.getcwd(), "data", "SPY_decision_vs_reality_momentum.csv")

    print("\n[Module F-M] Loading signals + trades...")
    ...
    print(f"\n✅ Saved decision vs reality (momentum) to: {out_path}")
    print(f"Rows evaluated (ORB-confirmed sessions): {len(result)}")

    if result.empty:
        print("\nNo ORB-confirmed days found to evaluate.")
        return
    ...
    print(f"\nDirectional opportunity rate (hit +1R within horizon): {directional_opportunity_rate:.2%}")
    ...
    print("\nNext: compare Momentum vs Conservative summaries side-by-side and then run $10,000 equity simulation.\n")
