...
def print_start_banner(cfg, run_date, tz, universe_file, symbol_count, session_open=None, session_close=None):
    print("=" * 90)
    print(f"LIVE TRACKER STARTED | {run_date} | TZ: {tz}")
    print(f"Universe: {universe_file} | Symbols: {symbol_count}")
    if session_open and session_close:
        print(f"Session (RTH): {session_open} -> {session_close}")
    print("-" * 90)

    print("STRATEGY")
    print(f"  EMA Period: {cfg.EMA_PERIOD}")
    print(f"  Cross Lookback (trading days): {cfg.CROSS_LOOKBACK_DAYS}")
    print(f"  Primary Window Days: {cfg.WINDOW_DAYS_PRIMARY}")
    print(f"  Secondary Window: {'ON' if cfg.ENABLE_SECONDARY_WINDOW else 'OFF'}"
          f"{' (' + str(cfg.WINDOW_DAYS_SECONDARY) + 'D)' if cfg.ENABLE_SECONDARY_WINDOW else ''}")
    print(f"  Rearm on Re-entry: {cfg.REARM_ON_REENTRY} | Reentry Mode: {cfg.REENTRY_MODE}")

    print("LIVE SETTINGS")
    print(f"  Interval: {cfg.LIVE_INTERVAL} | Poll: {cfg.LIVE_POLL_SECONDS}s | Last Completed Bar: {cfg.LIVE_USE_LAST_COMPLETED_BAR}")
    print(f"  Session Mode: {cfg.LIVE_SESSION_MODE} | Auto-wait: {cfg.LIVE_AUTO_WAIT_FOR_SESSION_START} | Auto-stop: {cfg.LIVE_AUTO_STOP_AFTER_SESSION_END}")

    print("NOTIFICATIONS")
    print(f"  Discord Enabled: {cfg.DISCORD_ENABLED} | Webhook set: {bool(cfg.DISCORD_WEBHOOK_URL)}")
    print(f"  Send LIVE alerts: {cfg.DISCORD_SEND_LIVE_ALERTS} | Send EOD summary: {cfg.DISCORD_SEND_EOD_SUMMARY}")

    print("SAFETY / STORAGE")
    print(f"  Ledger Enabled: {cfg.ENABLE_ALERTS_LEDGER} | Preserve empty alerts: {cfg.PRESERVE_EXISTING_ALERTS_FILE_IF_EMPTY}")
