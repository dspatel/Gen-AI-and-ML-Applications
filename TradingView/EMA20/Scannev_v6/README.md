...
## Startup / EOD Banners (Discord)

The live tracker and EOD scanner can optionally send a one-time configuration banner to Discord at start,
and a completion banner at the end of EOD. Control with these toggles in `config.py`:

- `DISCORD_SEND_STARTUP_BANNER`
- `DISCORD_SEND_EOD_BANNERS`
- `DISCORD_SEND_SHUTDOWN_BANNER`

Webhook must be provided via environment variable `DISCORD_WEBHOOK_URL`.
