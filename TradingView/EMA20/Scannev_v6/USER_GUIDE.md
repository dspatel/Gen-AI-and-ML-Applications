...
### Discord banner toggles

- `DISCORD_SEND_STARTUP_BANNER`: Send one-time LIVE startup banner (config summary).
- `DISCORD_SEND_EOD_BANNERS`: Send EOD start + completion banners.
- `DISCORD_SEND_SHUTDOWN_BANNER`: Send shutdown banner when LIVE exits.
- Set webhook via `DISCORD_WEBHOOK_URL` environment variable.
