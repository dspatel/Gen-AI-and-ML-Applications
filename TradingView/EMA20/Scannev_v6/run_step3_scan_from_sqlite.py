...
def _format_eod_start_message(run_date: str, universe_file: str, universe_count: int) -> str:
    return (
        f"🟪 **EMA20 EOD Scan STARTED ({CFG.DISCORD_ENV})**\n\n"
        f"**As-of Date:** {run_date}\n"
        f"**TZ:** {CFG.TIMEZONE}\n"
        f"**Universe:** `{os.path.basename(universe_file)}` ({universe_count} symbols)\n\n"
        f"**Strategy**\n"
        f"- EMA: {CFG.EMA_PERIOD}\n"
        f"- Cross lookback: {CFG.CROSS_LOOKBACK_DAYS} trading days\n"
        f"- Primary window: {CFG.WINDOW_DAYS_PRIMARY}D\n"
        f"- Secondary window: {'ON' if CFG.ENABLE_SECONDARY_WINDOW else 'OFF'}"
        f"{' (' + str(CFG.WINDOW_DAYS_SECONDARY) + 'D)' if CFG.ENABLE_SECONDARY_WINDOW else ''}\n"
        f"- Rearm on re-entry: {CFG.REARM_ON_REENTRY} ({CFG.REENTRY_MODE})"
    )


def _format_eod_done_message(run_date: str, cross_eligible: int, alerts: int, scan_all_path: str, alerts_path: str) -> str:
    return (
        f"🟩 **EMA20 EOD Scan COMPLETE ({CFG.DISCORD_ENV})**\n\n"
        f"**As-of Date:** {run_date}\n"
        f"**Cross-eligible:** {cross_eligible}\n"
        f"**Alerts:** {alerts}\n\n"
        f"**Files**\n"
        f"- scan_all: `{os.path.basename(scan_all_path)}`\n"
        f"- scan_alerts: `{os.path.basename(alerts_path)}`"
    )


def maybe_send_eod_start_banner(run_date: str, universe_file: str, universe_count: int) -> None:
    if getattr(CFG, "TEST_MODE", False):
        return
    if not (CFG.DISCORD_ENABLED and CFG.DISCORD_WEBHOOK_URL and getattr(CFG, "DISCORD_SEND_EOD_BANNERS", True)):
        return
    send_discord_message(CFG.DISCORD_WEBHOOK_URL, _format_eod_start_message(run_date, universe_file, universe_count))


def maybe_send_eod_done_banner(run_date: str, cross_eligible: int, alerts: int, scan_all_path: str, alerts_path: str) -> None:
    if getattr(CFG, "TEST_MODE", False):
        return
    if not (CFG.DISCORD_ENABLED and CFG.DISCORD_WEBHOOK_URL and getattr(CFG, "DISCORD_SEND_EOD_BANNERS", True)):
        return
    send_discord_message(CFG.DISCORD_WEBHOOK_URL, _format_eod_done_message(run_date, cross_eligible, alerts, scan_all_path, alerts_path))
...
def main() -> None:
    ...
    if getattr(CFG, "DISCORD_ENABLED", False) and getattr(CFG, "DISCORD_WEBHOOK_URL", "") and getattr(CFG, "DISCORD_SEND_EOD_SUMMARY", True):
        msg = f"EMA20 EOD Scan {asof} | Universe {len(symbols)} | Cross {len(cross_rows)} | Alerts {len(alerts)}"
        send_discord_message(CFG.DISCORD_WEBHOOK_URL, msg)

    maybe_send_eod_done_banner(run_date=today, cross_eligible=cross_eligible_count, alerts=alerts_count, scan_all_path=scan_all_path, alerts_path=alerts_path)
    conn.close()
