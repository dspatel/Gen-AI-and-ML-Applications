...
def build_horizon_results(
    ...
    for h in sorted(set(int(x) for x in horizons)):
        ...
        sessions_used = len(or_slice)
        missing = []
        # We can't know exact missing session dates here; caller can pass it via or_rows if present
        ref = build_reference_range(or_slice) if or_slice else {}
        overlap = compute_or_overlap_counts(or_slice) if or_slice else {"or_days":0,"or_overlap_days_count":0,"or_overlap_pairs_pct":0.0}
