...
def main() -> None:
    ...
    for sym in cfg.symbols:
        ...
        if len(sess) >= cfg.orb_bars:
            try:
                or_high, or_low, orb_len = build_opening_range(cfg, sess)
            except Exception as e:
                print(f"[{sym}] OR build error: {e}")
                continue

            st.or_high, st.or_low, st.or_ready = or_high, or_low, True
            # Terminal debug: show how many bars were used to build the OR window
            print(f"[{sym}] OR built using {getattr(st, '_orb_len_bars', orb_len)} bar(s) for {cfg.orb_minutes}m window @ {cfg.candle_minutes}m candles")
            try:
                st._orb_len_bars = int(orb_len)
            except Exception:
                pass
