...
def build_opening_range(cfg: Config, session_df: pd.DataFrame) -> tuple[float, float, int]:
    """
    Compute ORH/ORL using a **time window** from the session open.

    Why time-window vs bar-count:
      - With 15m candles and orb_minutes=30, we want the first TWO 15m bars.
      - Bar-count can break if the provider returns bars not aligned to the session open,
        or if your candle duration does not divide orb_minutes cleanly.
      - Using timestamps makes OR robust across 1m/5m/10m/15m candles.

    Assumption:
      - session_df is already filtered to the regular session and starts at the session open (or the first bar >= open).
      - time_local represents the candle START time (Yahoo finance behaves this way).

    Returns:
      (or_high, or_low, orb_len_bars)
    """
    if session_df.empty:
        raise RuntimeError("Session DF is empty; cannot build OR")

    session_start = session_df.iloc[0]["time_local"]
    or_end = session_start + pd.Timedelta(minutes=cfg.orb_minutes)

    orb = session_df[(session_df["time_local"] >= session_start) & (session_df["time_local"] < or_end)].copy()

    # Fallback: if for any reason the provider timestamps represent candle END time,
    # the last candle can equal or_end; include it.
    if orb.empty:
        orb = session_df[(session_df["time_local"] >= session_start) & (session_df["time_local"] <= or_end)].copy()

    if orb.empty:
        raise RuntimeError("OR window produced 0 bars; check timezone/session filtering")

    return float(orb["high"].max()), float(orb["low"].min()), int(len(orb))
...
def scan_catchup_events(
    ...
    armed = True
    # Start scanning AFTER the OR window ends (time-window based).
    session_start = session_df.iloc[0]["time_local"]
    or_end = session_start + pd.Timedelta(minutes=cfg.orb_minutes)

    # Number of bars in OR window (robust across candle sizes)
    orb_len = int(session_df[(session_df["time_local"] >= session_start) & (session_df["time_local"] < or_end)].shape[0])
    if orb_len <= 0:
        orb_len = cfg.orb_bars  # fallback
    i = orb_len
...
def process_latest_two_bars_live(cfg: Config, st: SymbolState, session_df: pd.DataFrame) -> Optional[BreakoutEvent]:
    ...
    if not st.or_ready:
        if len(session_df) >= cfg.orb_bars:
            st.or_high, st.or_low, orb_len = build_opening_range(cfg, session_df)
            st._orb_len_bars = orb_len
            st.or_ready = True
        else:
            return None
