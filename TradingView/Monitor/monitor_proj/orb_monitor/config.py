...
class Config:
    ...
    send_catchup_notifications: bool = True
    ...
    catchup_notify_limit_per_symbol: int = 5

    # Console
    show_dashboard: bool = True
    clear_console_each_tick: bool = False
    show_last_n_events: int = 3

    # Debug logging
    # If True, write per-symbol candle debug rows so you can verify exactly which bars
    # the system is pulling and evaluating (useful for timezone / 1-candle-shift issues).
    enable_candle_debug_log: bool = False

    # If True, continuously append *all pulled session candles* (open→now) to a per-symbol
    # CSV during LIVE/TEST so you can compare TradingView candles to what the system saw
    # in real time. This is independent of breakout logic.
    enable_raw_candle_log: bool = False
