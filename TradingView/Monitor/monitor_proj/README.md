...
### Candle debug logs (recommended when diagnosing missed/shifted alerts)

When `enable_candle_debug_log = True` (default), the monitor also writes a per-symbol candle trace:

- LIVE: `output/LIVE/candles_{SYMBOL}_YYYY-MM-DD_{candle}m.csv`
- TEST: `output/TEST/candles_{SYMBOL}_YYYY-MM-DD_{candle}m.csv`

Each row captures the exact **two closed bars** the strategy evaluated (breakout bar `b` and confirmation bar `c`),
along with OR levels, armed state, and an evaluation **reason** (e.g., `NO_BREAKOUT`, `DEDUPED`, `REARM_WAIT`, `BREAKOUT`).
