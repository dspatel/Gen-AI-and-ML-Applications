...
from orb_monitor.loggers import log_path, append_event, append_notify_decision
from orb_monitor.candle_debug import append_raw_session_candles
...
def notify_event(cfg: Config, ev) -> None:
    ...
    if getattr(ev, "is_catchup", False):
        ...
        if not cfg.send_catchup_notifications:
            append_notify_decision(cfg, getattr(ev, 'session_date', ''), sym, 'CATCHUP', 'SKIP_CFG', 'send_catchup_notifications=False')
            return
        ...
        if CATCHUP_NOTIFY_COUNTS[sym] > cfg.catchup_notify_limit_per_symbol:
            append_notify_decision(cfg, getattr(ev, 'session_date', ''), sym, 'CATCHUP', 'SKIP_LIMIT', f"limit={cfg.catchup_notify_limit_per_symbol}")
            return
        append_notify_decision(cfg, getattr(ev, 'session_date', ''), sym, 'CATCHUP', 'SEND', f"count={CATCHUP_NOTIFY_COUNTS[sym]}")
...
def main() -> None:
    ...
    for sym in cfg.symbols:
        ...
        # Raw candle stream log (open→now) for real-time debugging
        append_raw_session_candles(cfg, session_date_str, sym, sess)
