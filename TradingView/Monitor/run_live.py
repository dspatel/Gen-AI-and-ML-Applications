...
from orb_monitor.notify import notify_event_discord, notify_market_close_summary_discord
...
def main() -> None:
    ...
    if cfg.test_mode and cfg.test_date:
        ...
        df_spy = fetch_bars("SPY", cfg)
        if df_spy.empty:
            raise RuntimeError("No data returned for SPY; cannot infer session day.")
        session_day = df_spy["time_local"].max().tz_convert(tz).normalize()
    ...
    for sym in cfg.symbols:
        ...
        if len(sess) >= cfg.orb_bars:
            ...
            st.last_confirm_dt_processed = catchup_until
