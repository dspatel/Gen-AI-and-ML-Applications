...
## v1.1.2 Alert Layout (10-second scan)

Template lives in `config/notification_templates.yml` (`templates.default`).

**Line order**
1. 🕒 Breakout time
2. Decision (direction + confidence + key reasons)
3. 🧠 Story (one-glance summary)
4. Compact tags line
5. Ref/Inflation/Overlap/session counts
6. Intensity (close/wick/body/range %)
7. 🪜 Ladder (broken + not-broken horizons)
8. 🔶 Phase (Pre-OR vs Post-OR)

Edit wording/icons in:
- `config/notification_templates.yml`
- `interpretation/labels.yml` and `interpretation/label_engine.py`
