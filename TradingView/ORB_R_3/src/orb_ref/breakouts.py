...
def detect_breakouts_close_only_stepwise(
    ...
    # live_tracker passes params from YAML as a plain dict; unit tests may pass BreakoutParams.
    if isinstance(params, dict):
        raw: Dict[str, Any] = params
        # Be permissive: ignore unknown keys, coerce types, and fall back to defaults.
        p = BreakoutParams(
            close_required=bool(raw.get("close_required", BreakoutParams.close_required)),
            confirm_closes=int(raw.get("confirm_closes", BreakoutParams.confirm_closes)),
            inside_reset_pct=float(raw.get("inside_reset_pct", BreakoutParams.inside_reset_pct)),
            min_bars_between_alerts=int(
                raw.get("min_bars_between_alerts", BreakoutParams.min_bars_between_alerts)
            ),
        )
    else:
