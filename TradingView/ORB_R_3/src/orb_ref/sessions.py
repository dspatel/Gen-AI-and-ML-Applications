...
class TradingSessions:
    ...
    def _ensure_utc_ts(self, ts):
        """Ensure a timestamp is tz-aware in UTC (exchange_calendars versions vary)."""
        try:
            # pandas Timestamp
    ...
    def get_session_bounds(self, d: date) -> SessionBounds:
        ...
        open_utc = self._ensure_utc_ts(open_utc)
        close_utc = self._ensure_utc_ts(close_utc)
