...
def _last_complete_close_ts(now_cst: datetime, interval: str) -> pd.Timestamp:
    mins = _interval_minutes(interval)
    ts = pd.Timestamp(now_cst)
    # yfinance can show a partially forming candle; treat 'now-1min' as safe
    ts = ts - pd.Timedelta(minutes=1)
    return ts.floor(f"{mins}min")
...
def main() -> None:
    ...
    # If started before open: wait until open.
    ...
    print("=" * 70)
    print("LIVE: yfinance polling + DB-first RR + close-only breakout engine")
    ...
    state_by_sym: Dict[str, Dict[int, HorizonState]] = {sym: {} for sym in symbols}

    # Restore horizon armed-state to avoid duplicate alerts after a restart.
    # We disarm horizons that already broke earlier in the current phase (pre-OR vs post-OR).
    now_cst = datetime.now(CST)
    current_include_today_or = 0 if (now_cst < or_end_dt) else 1
    for sym in symbols:
        broken = load_broken_horizons(conn, sym, session_date_cst, interval, or_minutes, current_include_today_or)
        for h in broken.keys():
            state_by_sym[sym][int(h)] = HorizonState(armed=False)
    ...
    while True:
        ...
        last_complete = _last_complete_close_ts(now_cst, interval)
        ...
        for sym in symbols:
            ...
            for _, row in df.iterrows():
                res = evaluate_bar_close_only(
                    ...
                    rr_post=rr_post,
                    rr_seed=rr_seed,
                    state_by_h=state_by_sym[sym],
                    row=row,
                    or_end_ts_cst=pd.Timestamp(or_end_dt),
                )
