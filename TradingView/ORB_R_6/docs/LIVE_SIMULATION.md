...
### Live data pull window
Live only ingests up to the last fully closed candle close ("now-1min" floored to the interval), with a small +1 minute buffer on the API end time. This prevents yfinance/Yahoo errors when requesting bars that have not closed yet.
